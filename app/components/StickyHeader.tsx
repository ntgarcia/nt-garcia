"use client";

import Link from "next/link";
import { motion } from "framer-motion";
import { useHeroCollapsed } from "./HeroLogoProvider";

export default function StickyHeader() {
  const collapsed = useHeroCollapsed();

  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white px-4 md:px-13 py-4 md:py-6">
      <div className="mx-auto flex items-center justify-between">
        <Link href="/" className="flex items-center h-8">
          {collapsed && (
            <motion.img
              layoutId="site-signature"
              src="/signature.png"
              alt="Nathan Garcia"
              className="h-7 w-auto"
            />
          )}
        </Link>
        <nav className="flex items-center gap-6">
          <div className="flex items-center gap-6 ml-4">
            <a
              href="mailto:naninf.work@gmail.com"
              className="font-medium text-black underline transition-opacity hover:opacity-70"
            >
              Contact
            </a>
            <a
              href="https://www.instagram.com/nan____inf/"
              target="_blank"
              rel="noopener noreferrer"
              className="font-medium text-black underline transition-opacity hover:opacity-70"
            >
              Instagram
            </a>
            <a
              href="https://www.linkedin.com/in/ntns/"
              target="_blank"
              rel="noopener noreferrer"
              className="font-medium text-black underline transition-opacity hover:opacity-70"
            >
              LinkedIn
            </a>
            <a
              href="https://drive.google.com/file/d/1KPDohm-_RfX1UokEPrMhu2JRlBBkNl2l/view?usp=sharing"
              target="_blank"
              rel="noopener noreferrer"
              className="font-medium text-black underline transition-opacity hover:opacity-70"
            >
              CV
            </a>
          </div>
        </nav>
      </div>
    </header>
  );
}
